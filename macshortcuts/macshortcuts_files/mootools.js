...
var Tips = new Class({
	...
	build: function(el){
		el.myTitle = el.href ? el.href.replace(/http(s)*:\/\//, '') : (el.rel || false);
		if (el.title){
	...
	locate: function(event){
		...
		for (var z in prop){
			...
			if ((pos + tip[z] - scroll[z]) > win[z]) {
				...
				this.toolTip.addClass(this.options.className + '-pos' + z);
			} else {
				this.toolTip.removeClass(this.options.className + '-pos' + z);
			}
